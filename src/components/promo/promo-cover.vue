...
const bgImage = computed(() => {
  if (props.config?.bgImage) return getMediaUrl(props.config.bgImage)
  const promoAssets = Array.isArray(props.activity?.promoAssets) ? props.activity.promoAssets : []
  if (promoAssets.length && promoAssets[0]?.url) return getMediaUrl(promoAssets[0].url)
  const legacy = Array.isArray(props.activity?.assets) ? props.activity.assets : []
  if (legacy.length && legacy[0]?.url) return getMediaUrl(legacy[0].url)
  return ''
})
</script>
