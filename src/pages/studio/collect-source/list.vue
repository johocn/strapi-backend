<template>
  <view class="page-container">
    <PageHeader title="采集源">
      <button class="btn-primary" @click="goCreate" v-if="hasPermission('studio.collect-source.create')">+ 新增</button>
    </PageHeader>
      ...
      >
        <view class="data-info">
          ...
          <view class="data-meta">
            <text class="meta-item">🔗 {{ item.url || '无 URL' }}</text>
          ...
          <view class="data-footer">
            ...
            <text class="data-date">{{ item.lastCollectedAt || '' }}</text>
          </view>
        </view>
        <view class="data-actions">
          <view v-if="hasPermission('studio.collect-source.update')" class="action-btn edit" @click.stop="goEdit(item.documentId)">编辑</view>
          <view v-if="hasPermission('studio.collect-source.update')" class="action-btn delete" @click.stop="handleDelete(item)">删除</view>
        </view>
      </view>
    </view>
    ...
    <view v-if="!loading && dataList.length === 0" class="empty-state">
      ...
      <text class="empty-text">暂无采集源</text>
    </view>
...
const typeOptions = ['全部类型', '模板', '自定义']
const typeReverseMap = { 1: 'template', 2: 'custom' }
...
async function loadData(page = 1) {
  ...
  try {
    ...
    if (searchKeyword.value) {
      ...
      params['filters[$or][1][url][$contains]'] = searchKeyword.value
    }
    if (typeIndex.value > 0) {
      params['filters[type]'] = typeReverseMap[typeIndex.value]
    }
    const { list, pagination: pg } = await collectSourceApi.list(params)
    dataList.value = list
    pagination.value = pg
...
function goCreate() {
  uni.navigateTo({ url: '/pages/studio/collect-source/edit' })
}

function goEdit(id) {
  uni.navigateTo({ url: `/pages/studio/collect-source/edit?documentId=${id}` })
}

async function handleDelete(item) {
  uni.showModal({
    title: '确认删除',
    content: `确定要删除采集源「${item.name}」吗？`,
    success: async (res) => {
      if (res.confirm) {
        try {
          await collectSourceApi.delete(item.documentId)
