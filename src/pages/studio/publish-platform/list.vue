<template>
  <view class="page-container">
    <PageHeader title="发布平台">
      <button class="btn-primary" @click="goCreate" v-if="hasPermission('studio.publish-platform.create')">+ 新增</button>
    </PageHeader>
    ...
    <view class="data-list">
      ...
      >
        ...
        <view class="data-actions">
          <view v-if="hasPermission('studio.publish-platform.update')" class="action-btn edit" @click.stop="goEdit(item.documentId)">编辑</view>
          <view v-if="hasPermission('studio.publish-platform.update')" class="action-btn delete" @click.stop="handleDelete(item)">删除</view>
        </view>
      </view>
    </view>
    ...
    <view v-if="!loading && dataList.length === 0" class="empty-state">
      ...
      <text class="empty-text">暂无发布平台</text>
    </view>
...
import { publishPlatformApi } from '../../../api/studio.js'
...
function goCreate() {
  uni.navigateTo({ url: '/pages/studio/publish-platform/edit' })
}

function goEdit(id) {
  uni.navigateTo({ url: `/pages/studio/publish-platform/edit?documentId=${id}` })
}

async function handleDelete(item) {
  uni.showModal({
    title: '确认删除',
    content: `确定要删除平台「${item.name}」吗？`,
    success: async (res) => {
      if (res.confirm) {
        try {
          await publishPlatformApi.delete(item.documentId)
