<template>
  <view class="page-container">
    ...
    <view class="data-list">
      <view v-for="item in dataList" :key="item.documentId" class="data-card">
        <view class="data-info">
          ...
          <view class="data-meta">
            <text class="meta-item">回调: {{ shortText(item.redirect_uri, 30) }}</text>
          </view>
        ...
        <view class="data-actions">
          <view v-if="hasPermission('sso.auth-code.delete')" class="action-btn delete" @click.stop="handleDelete(item)">删除</view>
        </view>
      </view>
    </view>
    ...
    <view v-if="!loading && dataList.length === 0" class="empty-state">
      ...
      <text class="empty-text">暂无授权码记录</text>
    </view>
...
import { ssoAuthCodeApi } from '../../../api/sso.js'
...
async function loadData(page = 1) {
  ...
  try {
    ...
    if (searchKeyword.value) {
      params['filters[$or][0][code][$contains]'] = searchKeyword.value
      params['filters[$or][1][app_code][$contains]'] = searchKeyword.value
    }
    const { list, pagination: pg } = await ssoAuthCodeApi.list(params)
    dataList.value = list
    pagination.value = pg
...
function handleDelete(item) {
  uni.showModal({
    title: '确认删除',
    content: `确定要删除授权码「${item.code}」吗？`,
    success: async (res) => {
      if (res.confirm) {
        try {
          await ssoAuthCodeApi.delete(item.documentId)
