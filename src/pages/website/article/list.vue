<template>
  <view class="page-container">
    <PageHeader title="资讯文章">
      <button class="btn-primary" @click="goCreate" v-if="hasPermission('article.create')">+ 新增文章</button>
    </PageHeader>
      ...
      >
        <view class="article-info">
          ...
          <view class="article-meta">
            <text class="meta-item" v-if="item.category">📂 {{ item.category?.name || item.category }}</text>
            <text class="meta-item">📝 {{ item.excerpt ? item.excerpt.substring(0, 50) : '' }}</text>
          </view>
          <view class="article-footer">
            <view class="article-status" :class="item.status">{{ getStatusText(item.status) }}</view>
            <view class="article-date">{{ formatDate(item.publishedAt || item.createdAt) }}</view>
          </view>
        </view>
        <view class="article-actions">
          <view v-if="(item.status === 'draft') && hasPermission('article.publish')" class="action-btn publish" @click.stop="handlePublish(item)">发布</view>
...
import { getTagGroupList } from '../../../api/tag.js'
...
function goEdit(id) {
  uni.navigateTo({ url: `/pages/website/article/edit?documentId=${id}` })
}
...
async function handleDelete(item) {
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        try {
          await articleApi.delete(item.documentId)
...
async function handlePublish(item) {
  uni.showModal({
    title: '确认发布',
    content: `确定要发布文章「${item.title}」吗？`,
    success: async (res) => {
      if (res.confirm) {
        try {
          await articleApi.publish(item.documentId)
...
async function handleArchive(item) {
  uni.showModal({
    title: '确认下架',
    content: `确定要下架文章「${item.title}」吗？`,
    success: async (res) => {
      if (res.confirm) {
        try {
          await articleApi.archive(item.documentId)
