<template>
  <view class="page-container">
    ...
    <view class="progress-list">
      ...
      >
        <view class="progress-info">
          ...
          <view class="progress-course">{{ item.course?.title || '未知课程' }}</view>
          ...
          <view class="progress-meta">
            <text class="meta-item">{{ item.completedLessons || 0 }}/{{ item.totalLessons || 0 }} 课时</text>
    ...
    <view v-if="!loading && progressList.length === 0" class="empty-state">
      ...
      <text class="empty-text">暂无学习进度</text>
    </view>
...
import { getCourseProgressList } from '../../src/api/course.js'
...
async function loadData(page = 1) {
  ...
  try {
    ...
    const { list, pagination: pg } = await getCourseProgressList(params)
...
.progress-course { font-size: 26rpx; color: #666; margin-bottom: 16rpx; }
