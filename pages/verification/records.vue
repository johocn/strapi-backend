    ...
    <view class="list">
      ...
      >
        <view class="item-header">
          <view class="item-type">{{ item.verificationType }}</view>
          <view class="item-status" :class="item.status">{{ item.status }}</view>
        </view>
        <view class="item-code">验证码: {{ item.verificationCode }}</view>
        <view class="item-meta">
          <text class="meta-tag">用户: {{ item.userId }}</text>
          <text class="meta-tag" v-if="item.verifiedAt">验证: {{ formatDate(item.verifiedAt) }}</text>
...
import { getVerificationRecordList } from '../../src/api/points.js'
...
async function loadData() {
  try {
    ...
    const res = await getVerificationRecordList(params)
    list.value = res.list
